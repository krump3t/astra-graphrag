...
def test_curve_embedding_includes_well_id():
    ...
    if not curve_node:
        print("  [SKIP] force2020-curve-1 not found")
        return
    ...
    print("  [PASS] Curve embedding includes well relationship")
...
def test_well_embedding_includes_curve_count():
    ...
    if not well_node:
        print("  [SKIP] force2020-well-15_9-13 not found")
        return
    ...
    print("  [PASS] Well embedding includes curve count")
    print(f"     Preview: {text[:150]}...")
...
def test_curve_embedding_includes_well_name():
    ...
    if '_well_name' in curve_node.get('attributes', {}):
        ...
        print("  [PASS] Curve embedding includes well name")
    else:
        print("  [SKIP] Well has no name attribute")
...
def test_well_embedding_includes_curve_types():
    ...
    if '_curve_mnemonics' in well_node.get('attributes', {}):
        ...
        print("  [PASS] Well embedding includes curve types")
    else:
        print("  [SKIP] No curve mnemonics found")
...
def test_embedding_without_edges_still_works():
    ...
    print("  [PASS] Backwards compatible (works without edges)")
...
def test_relationship_enhancement_for_all_force2020_curves():
    ...
    if not force2020_curves:
        print("  [SKIP] No FORCE 2020 curves found")
        return
    ...
    if missing_relationships:
        print(f"  [FAIL] {len(missing_relationships)} curves missing relationship info")
        print(f"     Sample: {missing_relationships[:5]}")
    else:
        print(f"  [PASS] All {len(force2020_curves[:50])} tested curves have relationship info")
...
def test_relationship_searchability():
    ...
    if not curve_edges:
        print("  [SKIP] No curves found for well")
        return
    ...
    if curve_node:
        ...
        print("  [PASS] Curves searchable by well ID")
